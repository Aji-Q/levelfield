...
| `elevenlabs.sh` | ElevenLabs Text to Speech | `ELEVENLABS_API_KEY` env var | LevelField 成片默认；参数与 `production-plan.md` 对齐 |
| `say.sh` | macOS offline preview | none | 仅用于自动模式与时长 QA；不作为最终成片音色 |

LevelField 项目额外内置 ElevenLabs adapter。其它后端的代码片段在下面，
