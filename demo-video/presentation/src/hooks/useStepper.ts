 ...
 */
const STORAGE_KEY = "levelfield-demo-cursor-v1";
...
export function useStepper(chapters: ChapterDef[]): StepperState {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // In Auto mode Space belongs exclusively to AutoStartGate. Letting the
      // manual stepper handle the same key would skip the opening narration.
      if (
        e.key === " " &&
