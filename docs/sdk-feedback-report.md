...
(see the review's Business-Impact analysis) — a more complex pitch than "here's our tool scoring
...
| 5 (positive) | `SomniaMarkets` accepts an indexer-only, signerless config for read-only use — confirmed directly this session (`new SomniaMarkets({ indexerUrl, chain: somniaShannon })`, no `privateKey`, worked exactly as documented). | Worth stating explicitly in the docs, which currently only show full trading configs. |
